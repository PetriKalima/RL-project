...
for i in range(0,episodes):
    ...
    if player.memory.__len__()<1000:
        if (rew1 ==-10 and cum_reward>-10) or (rew1 ==10 and  cum_reward>10) : 
           for k in range(len(statesP)):
              player.store_transition(statesP[k], actionsP[k], ob1sP[k], rew1sP[k], donesP[k])
    else:   
    
        if done and rew1 ==10 and  cum_reward>10 and (flagwin_epoNotadded==True):
           for k in range(len(statesP)):
              player.store_transition(statesP[k], actionsP[k], ob1sP[k], rew1sP[k], donesP[k])
           flagwin_epoNotadded = False

        if done and rew1 ==-10 and cum_reward>-10 and (flagwin_epoNotadded==False ): 
           for k in range(len(statesP)):
              player.store_transition(statesP[k], actionsP[k], ob1sP[k], rew1sP[k], donesP[k])
